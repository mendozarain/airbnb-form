import { launch, limits, sessions } from "@cloudflare/playwright";
import { Buffer } from "node:buffer";
import type { AdminSubmission } from "@cozy-d-714/shared";
...
export type GoogleFormSubmission = AdminSubmission & {
...
export async function getBrowserLivePreview(env: Env) {
  if (!env.CLOUDFLARE_ACCOUNT_ID || !env.CLOUDFLARE_API_TOKEN) {
    return { available: false, error: "Cloudflare API credentials are not configured." };
  }

  const activeSessions = await sessions(env.BROWSER);
  const activeSession = activeSessions.find((session) => session.connectionId) ?? activeSessions[0];

  if (!activeSession) {
    return { available: false, error: "No active Browser Run session right now." };
  }

  const targets = await listBrowserTargets(env, activeSession.sessionId);
  const target = targets.find((item: any) => item.devtoolsFrontendUrl) ?? targets[0];

  if (!target?.devtoolsFrontendUrl) {
    return { available: false, sessionId: activeSession.sessionId, error: "Live preview is not ready yet. Try again in a few seconds." };
  }
...
export async function submitGoogleForm(env: Env, submission: GoogleFormSubmission): Promise<GoogleFormResult> {
  // Django analogy: think of this as a Celery task body. The Hono route calls it,
  // but the work itself is isolated so we can later move it to a queue/cron safely.
  const storageState = await loadGoogleStorageState(env);

  if (!storageState) {
    return { ok: false, error: "Google browser session is not connected. Open Settings and connect Google first." };
  }

  const browserLimits = await limits(env.BROWSER);
  if (browserLimits.allowedBrowserAcquisitions < 1) {
    return {
      ok: false,
      retryable: true,
      error: `Cloudflare Browser Run is cooling down. Try again in ${formatWait(browserLimits.timeUntilNextAllowedBrowserAcquisition)}.`
    };
  }
  ...
  try {
    await page.goto(env.GOOGLE_FORM_URL, { waitUntil: "networkidle" });
    ...
    await env.ID_BUCKET.put("google/storage-state.json", JSON.stringify(updatedStorageState), {
    ...
    return { ok: false, retryable: message.includes("429") || message.includes("Rate limit"), error: `[${AUTOMATION_VERSION}] ${message}` };
  } finally {
    await browser.close();
  }
}

async function fillTextByLabel(page: Awaited<ReturnType<typeof launch>> extends infer _ ? any : never, label: string, value: string) {
  const field = page.getByText(label).locator("xpath=ancestor::div[@role='listitem']").getByRole("textbox").first();
  await field.fill(value);
}

async function fillNthShortAnswer(page: any, questionNumber: number, value: string) {
  const field = page.getByText(new RegExp(`^${questionNumber}\\.?$`)).locator("xpath=ancestor::div[@role='listitem']").getByRole("textbox").first();
  await field.fill(value);
}

async function fillDateByLabel(page: any, label: string, value: string) {
  const field = page.getByText(label).locator("xpath=ancestor::div[@role='listitem']").locator("input[type='date'], input[type='text']").first();
...
async function uploadIdFiles(page: any, files: GoogleFormFile[]) {
  const uploadQuestion = page.locator('div[role="listitem"]').filter({
  ...
  for (const fileChunk of chunks) {
    ...
    await uploadIdFileChunk(page, uploadQuestion, fileChunk, uploadedFilenames, uploadTimeoutMs, fileChunk === chunks[0]);
  }

  await closeUploadPickerIfOpen(page);
}

async function uploadIdFileChunk(page: any, uploadQuestion: any, files: GoogleFormFile[], expectedFilenames: string[], uploadTimeoutMs: number, openPicker: boolean) {
...
async function clickPickerPrimaryAction(page: any, dialog: any, timeoutMs = 30_000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const clickedInFrame = await clickVisibleTextButtonMatchingInAnyFrame(page, /^(Insert|Select|Done)$/i, 2_000).catch(() => false);
...
async function waitForUploadChunkOutcome(page: any, dialog: any, uploadQuestion: any, filenames: string[], timeoutMs: number) {
  const startedAt = Date.now();
  let enterPresses = 0;
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (attachedCount >= filenames.length && (!pickerVisible || await isPickerReadyForMoreFiles(page))) {
      return;
    }
  ...
  const attachedCount = await countAttachedFiles(uploadQuestion, filenames);
  throw new Error(`Google Drive picker did not finish attaching files. Attached ${attachedCount}/${filenames.length}.`);
}
...
async function waitForUploadToAttach(page: any, dialog: any, uploadQuestion: any, filenames: string[], timeoutMs: number) {
  const attachedFile = filenames[0] ? uploadQuestion.getByText(filenames[0]).first() : uploadQuestion.getByText(/\.(jpg|jpeg|png|pdf)$/i).first();
  const startedAt = Date.now();
...
async function isUploadPickerVisible(page: any) {
  return page.evaluate(() => {
    ...
    return dialogs.some((dialog) => {
      ...
      return rect.width > 0 && rect.height > 0 && /Insert file|Upload up to|drag files|My Drive|Browse/i.test(text);
    });
  }).catch(() => false);
}
...
async function isPickerReadyForMoreFiles(page: any) {
  for (const frame of page.frames()) {
    const ready = await frame.evaluate(() => {
      ...
      const browse = Array.from(document.querySelectorAll("button, [role='button'], span, div")).some((element) => {
        const text = element.textContent?.trim();
        const rect = element.getBoundingClientRect();
...
async function waitForPickerUploadComplete(page: any, expectedCount: number, timeoutMs: number) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const frameStates = await Promise.all(page.frames().map((frame: any) => {
      return frame.evaluate(() => {
        const bodyText = document.body?.innerText ?? "";
        const uploadedCount = (bodyText.match(/\buploaded\b/gi) ?? []).length;
        const hasProgress = Boolean(document.querySelector('[role="progressbar"], mat-progress-bar, .progress, [aria-valuenow]'));
    ...
    const state = frameStates.reduce((combined: { uploadedCount: number; hasProgress: boolean; hasUploadingText: boolean }, current: { uploadedCount: number; hasProgress: boolean; hasUploadingText: boolean }) => ({
    ...
    if (state.uploadedCount >= expectedCount || (!state.hasProgress && !state.hasUploadingText && state.uploadedCount > 0)) {
      return;
    }
    ...
    for (const control of controls) {
      ...
      if (visible) {
        ...
        if ((target as HTMLElement).dataset.cozyD714ScreenshotVisibility === undefined) {
          (target as HTMLElement).dataset.cozyD714ScreenshotVisibility = (target as HTMLElement).style.visibility;
        }
...
async function waitForGoogleFormSubmission(page: any) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (/Your response has been recorded|Thanks for filling out this form|Here's what was received/i.test(state.bodyText)) {
      return;
    }

    if (state.requiredErrors.length > 0) {
      throw new Error(`Google Form did not submit because a required field is still missing: ${state.requiredErrors.join("; ")}`);
    }
...
async function clickQuestionCheckbox(page: any, questionText: RegExp, name: string) {
  const question = page.locator('div[role="listitem"]').filter({
  ...
  if ((await checkbox.count().catch(() => 0)) > 0) {
    ...
    await question.getByText(questionText).click({ force: true, timeout: 10_000 }).catch(() => undefined);
  }
  ...
  await question.locator("label, span, div").filter({ hasText: questionText }).first().click({ force: true, timeout: 10_000 }).catch(() => undefined);
  await page.waitForTimeout(300);
  ...
  const clickedByDom = await question.evaluate((element: Element) => {
...
async function clickCheckboxBySurroundingText(page: any, questionText: RegExp) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const state = await page.evaluate(({ source, flags }: { source: string; flags: string }) => {
...
async function isCheckboxCheckedBySurroundingText(page: any, questionText: RegExp) {
  ...
  return page.evaluate(({ source, flags }: { source: string; flags: string }) => {
    const pattern = new RegExp(source, flags);
    const checkboxes = Array.from(document.querySelectorAll('[role="checkbox"], input[type="checkbox"]'));
...
async function removeAttachedFiles(page: any, uploadQuestion: any, keepCount: number) {
  ...
  for (let index = 0; index < 10; index += 1) {
    const result = await uploadQuestion.evaluate((root: Element, keepCount: number) => {
      function isVisible(element: Element) {
        const rect = element.getBoundingClientRect();
        return rect.width > 0 && rect.height > 0;
      }
      ...
      const controls = Array.from(lastChip.querySelectorAll("button, [role='button'], [aria-label], [data-tooltip]"));
      const removable = controls.reverse().find((element) => {
        ...
        return isVisible(element) && (/remove|delete|close/i.test(label) || /^[x×✕]$/i.test((element.textContent ?? "").trim()));
      });

      const fallback = Array.from(root.querySelectorAll("button, [role='button'], [aria-label], [data-tooltip]")).reverse().find((element) => {
        const label = [
          element.getAttribute("aria-label"),
          element.getAttribute("data-tooltip"),
          element.getAttribute("title"),
        ...
        return isVisible(element) && (/remove|delete/i.test(label) || /^[x×✕]$/i.test((element.textContent ?? "").trim()));
      });
    ...
    }, keepCount).catch(() => ({ removed: false, count: 0 }));
...
async function hasExpectedAttachedFiles(uploadQuestion: any, filenames: string[]) {
  if (filenames.length <= 1) {
    return (await uploadQuestion.getByText(/\.(jpg|jpeg|png|pdf)$/i).first().isVisible().catch(() => false))
      || (await countAttachedFiles(uploadQuestion, filenames) >= 1);
  }
...
async function countAttachedFiles(uploadQuestion: any, filenames: string[]) {
  return uploadQuestion.evaluate((root: Element, filenames: string[]) => {
    const clone = root.cloneNode(true) as Element;
    clone.querySelectorAll('[role="dialog"], div[aria-modal="true"], iframe').forEach((element) => element.remove());
    ...
    const removeControls = Array.from(clone.querySelectorAll("button, [role='button'], [aria-label], [data-tooltip]")).filter((element) => {
      const label = [
        element.getAttribute("aria-label"),
        element.getAttribute("data-tooltip"),
        element.getAttribute("title"),
    ...
    const chips = Array.from(clone.querySelectorAll("div, span")).filter((element) => {
      ...
      return isVisible(element)
        && rect.width > 40
        && rect.height > 16
        && (/\.?(jpg|jpeg|png|pdf)\b/i.test(text) || /Add file/i.test(text) === false && /Remove|delete|close|×|✕/i.test(text));
    });
...
async function clickVisibleTextButtonMatchingInAnyFrame(page: any, textPattern: RegExp, timeoutMs: number) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    for (const frame of page.frames()) {
      const clicked = await frame.evaluate(({ source, flags }: { source: string; flags: string }) => {
        const pattern = new RegExp(source, flags);
        const candidates = Array.from(document.querySelectorAll("button, [role='button'], span, div"));
        const target = candidates.find((element) => {
          const text = element.textContent?.trim() ?? "";
...
async function setFilesInAnyFrame(page: any, payloads: Array<{ name: string; mimeType: string; buffer: Buffer }>, timeoutMs: number, setInputTimeoutMs: number) {
  const startedAt = Date.now();
...
async function listBrowserTargets(env: Env, sessionId: string) {
  const response = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/browser-rendering/devtools/browser/${sessionId}/json/list`,
    {
      headers: {
  ...
  if (!response.ok) {
    throw new Error(`Could not load Browser Run live preview: ${response.status}`);
  }

  const payload: any = await response.json();
  return Array.isArray(payload) ? payload : payload.result ?? payload.targets ?? [];
}
